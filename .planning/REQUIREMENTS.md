...
- **SETUP-01**: The system shall provide a monolithic repository containing a Next.js frontend (`apps/web`), Express backend (`apps/api`), and PostgreSQL database configuration.
- **SETUP-02**: The database shall manage relational schemas with migrations, transactional support, and numeric data types for all monetary amounts.
- **AUTH-01**: Users shall authenticate via username and password with bcrypt hashing and session-based persistent login.
- **AUTH-02**: The frontend shall provide an idle-timeout PIN-lock overlay that locks the interface without terminating the underlying session.
...
- **RBAC-02**: The backend shall enforce granular permission checks on all API endpoints, and the frontend shall dynamically render navigation and actions based on user permissions.
- **UI-01**: The application shall deliver a mobile-first, touch-friendly UI framework with large touch cards, high-contrast layouts, and an integrated on-screen numeric keypad.
...
- **FONT-01**: Product cards, screens, and receipts shall render Urdu typography using the Jameel Noori Nastaleeq font.

### Phase 3: Core Billing Engine (Standard Products)
- **BILL-01**: The billing screen shall provide touch-card product selection with dual calculation modes: Weight → Amount (`qty_kg * rate`) and Amount → Weight (`amount / rate`).
- **BILL-02**: The billing screen shall support entering an explicit discounted final amount, calculating `discount = subtotal - final_amount`, restricted to users with `can_discount` permission.
